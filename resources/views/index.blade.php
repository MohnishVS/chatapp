...
<script type="text/javascript">
    ...
    setInterval("displayrec();", 3000);
    };
    function displayrec(){
        if(true){
        $( "#messagebox" ).load( "/index #messagebox");
        }
    };
  </script>
@endsection
    ...
    <div class="card-body">
        ...
        <div class="row position-relative overflow-auto">
            <div class="col-6 receiver" >
                <?php $users = DB::table('messages')->where([['user_id', session('rec_id')], ['rec_id', session('user_id')]])->pluck('message');
                    foreach ($users as $recmsg) {?>
                <label for="recmsg" id="recmsg">{{$recmsg}}</label>
                <?php echo "<br>"; } ?>
            </div>
            <div class="col-6 sender">
                    <?php
                        // use Illuminate\Support\Facades\DB;
                        $users = DB::table('messages')->where([['user_id', session('user_id')] , ['rec_id', session('rec_id')]])->pluck('message');
        ...
        <div class="container fixed-bottom p-4">
            &nbsp;
                <div class="form-group">
                        ...
                        onclick="displayrec();" autocomplete="off">
